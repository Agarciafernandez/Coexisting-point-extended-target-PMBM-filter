function [d_gospa, x_to_y_assignment, decomposed_cost] =  GOSPA_point_extended(x_mat, y_mat, p, c, alpha,c_b)
% AUTHOR: Abu Sajana Rahmathullah
%Modified by Angel Garcia-Fernandez to account for point-extended targets

%c_b must be smaller than c.
...
for ix = 1:nx
    for iy = 1:ny
        ...
        x_pos=x_mat(ix,1).x(1:2);
        X=x_mat(ix,1).X;
        
        y_pos=y_mat(iy,1).x(1:2);
        Y=y_mat(iy,1).X;
        
        if(isempty(X) && isempty(Y))
            %Both are point targets
            cost_mat(ix, iy)= min(computeBaseDistance(x_pos, y_pos), c_b); %Cut-off c_b
        elseif(~isempty(X) && ~isempty(Y))
            %Both are extended targets
            cost_mat(ix, iy) ...
                = min(GaussianWassersteinDistance(x_pos,X,y_pos,Y), c_b); %Cut-off c_b
        else
            %One is a point target and the other and extended target
            cost_mat(ix, iy) = c_b;
        end
    end
end
